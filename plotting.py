...
def get_animation(simulation_directory, timeste_ps, time_scaling, length_scaling, variable, figure_properties,
    ...
    def plot_data(variable_data):
        if multiple_var is True:
            im1 = ax.pcolormesh(X1, X2, variable_data[0], shading='flat', vmin=vmin, vmax=vmax);
            if mirror_horizontally is True:
                im1_mirror = ax.pcolormesh(X1, -X2, variable_data[0], shading='flat', vmin=vmin, vmax=vmax);
            X1_2_temp = X1
            X2_2_temp = X2
            ...
            if cmap2 is not None:
                im2 = ax.pcolormesh(-X1_2_temp, X2_2_temp, variable_data[1], shading='flat', vmin=vmin2, vmax=vmax2, cmap=cmap2)
                if mirror_horizontally is True:
                    im2_mirror = ax.pcolormesh(-X1_2_temp, -X2_2_temp, variable_data[1], shading='flat', vmin=vmin2, vmax=vmax2, cmap=cmap2)
            else:
                im2 = ax.pcolormesh(-X1_2_temp, X2_2_temp, variable_data[1], shading='flat', vmin=vmin2, vmax=vmax2)
                if mirror_horizontally is True:
                    im2_mirror = ax.pcolormesh(-X1_2_temp, -X2_2_temp, variable_data[1], shading='flat', vmin=vmin2, vmax=vmax2)
            if mirror_horizontally is True:
                return (im1, im2, im1_mirror, im2_mirror)
            else:
                return (im1, im2)
        else:
            return ax.pcolormesh(X1, X2, variable_data, shading='flat', vmin=vmin, vmax=vmax);
    ...
    if draw_timebar is True:
        tax = div.append_axes('bottom', size='15%', pad = figure_properties.timebar_padding)
        ...
        tax.tick_params(axis='y', which='both', right='off', left='off', labelleft='off')
