...
def test_default_viriality():
    ...
    z = 0
    mp = MakinoProfile(mass, z, cosmo=cosmology.Planck15, concentration_method='klypin-planck-relaxed')
    kp = KingProfile(mass, z, cosmo=cosmology.Planck15, concentration_method='klypin-planck-relaxed')
...
def test_concentration_methods():
    ...
    conc_methods = ['dolag', 'bullock', 'klypin-planck-all', 'klypin-planck-relaxed', 'klypin-wmap-all', 'klypin-wmap-relaxed', 'dutton', 'maccio']

    mass = (10 ** 12.5) * u.M_sun
    z = 0

    for cm in conc_methods:
        mp = MakinoProfile(mass, z, cosmo=cosmology.Planck15, concentration_method=cm)
        kp = KingProfile(mass, z, cosmo=cosmology.Planck15, concentration_method=cm)
...
def test_wrong_concentration_method():
    ...
    try:
        mass = (10 ** 12.5) * u.M_sun
        z = 0
        mp = MakinoProfile(mass, z, cosmo=cosmology.Planck15, concentration_method='invalid')
        assert 0
    except ValueError:
        pass
    try:
        kp = KingProfile(mass, z, cosmo=cosmology.Planck15, concentration_method='invalid')
        assert 0
    except ValueError:
        pass
