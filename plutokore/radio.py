...
def get_A(q):
    return (
        _sp.gamma(q / 4.0 + 19.0 / 12.0)
        * _sp.gamma(q / 4.0 - 1.0 / 12.0)
        * _sp.gamma(q / 4.0 + 5.0 / 4.0)
    ) / (_sp.gamma(q / 4.0 + 7.0 / 4.0))


def get_K(q, gamma_c):
    return (
        get_A(q)
        * (1.0 / ((gamma_c - 1) ** ((q + 5.0) / 4.0)))
        * (2.0 * _np.pi / 3.0) ** (-(q - 1) / 2.0)
...
def get_L0(
    ...
    log_eta_term = ((q + 1) / 4.0) * _np.log10(eta) + (-(q + 5.0) / 4.0) * _np.log10(
        1.0 + eta
    )
    log_mu0_term = ((q + 1) / 4.0) * _np.log10(2 * _const.mu0.si.value)
    log_nu_term = (-(q - 1) / 2.0) * _np.log10(
        freq.si.value
        * (_const.m_e.si.value ** 3.0)
        * (_const.c.si.value ** 4.0)
...
def clamp_tracers(
    simulation_data, ntracers, tracer_threshold=1e-7, tracer_effective_zero=1e-10
):
...
def clamp_tracers_internal(tracers, tracer_threshold=1e-7, tracer_effective_zero=1e-10):
...
def get_luminosity_old(
    ...
    kpc_per_arcsec = _cosmo.kpc_proper_per_arcmin(redshift).to(_u.kpc / _u.arcsec)
    ...
    (radio_tracer_mask, clamped_tracers, radio_combined_tracers) = clamp_tracers(
    ...
    if calculate_luminosity is True:
        radio_luminosity = (
            ...
            * radio_cell_volumes_physical
            / vol_scale
        ).to(_u.W / _u.Hz)
        radio_luminosity_tracer_weighted = (
            radio_luminosity * radio_tracer_mask * clamped_tracers
        )

    flux_const_term = (L0 / (4 * _np.pi * (Dlumin ** 2))) * (
        (1 + redshift) ** (1 + alpha)
    )
    ...
    flux_density = (
        flux_const_term * flux_prs_term * flux_vol_term * flux_beam_term
    ).to(_u.Jy)
    ...
    if convolve_flux is True:
        ...
        flux_density_tracer_weighted = (
            _convolve(
                flux_density_tracer_weighted.to(_u.Jy), beam_kernel, boundary="extend"
            )
            * _u.Jy
        )
...
def get_flux_density(radio_luminosity, redshift, alpha=0.6):
    ...
    flux_density = (
        (radio_luminosity / (4 * _np.pi * (Dlumin ** 2)))
        * ((1 + redshift) ** (1 + alpha))
    ).to(_u.Jy)
...
def get_convolved_flux_density(flux_density, redshift, beam_FWHM_arcsec):
    ...
    flux_density = (
        _convolve(flux_density.to(_u.Jy), beam_kernel, boundary="extend") * _u.Jy
    )
...
def get_surface_brightness(
    flux_density, simulation_data, unit_values, redshift, beam_FWHM_arcsec
):
...
def calculate_volume_emissivity_coefficient(
    *,
    q=2.2,
    gamma_c=4.0 / 3.0,
    eta=0.1,
...
def calculate_3d_sb(
    *,
    ...
    B0 = (
        j0
        * _np.power(freq / freq0, freq_exp)
        / (4 * _np.pi * _np.power(p0, pressure_exp))
    )
    # calculate tracer mask
    tracer_mask = _np.where(
        _np.greater(sim_data.tr1, tracer_threshold), 1, small_tracer_val
    )

    # integrate our pressure
    integrated_pressure_term = _trapz(
        y=_np.power(_np.multiply(sim_data.prs, tracer_mask), pressure_exp),
        x=sim_data.my * sim_data.unit_length,
        axis=1,
    )
    integrated_pressure_term = integrated_pressure_term * _np.power(
        sim_data.unit_pressure, pressure_exp
    )
...
def regrid_3d_sb(
    *, sim_data, sb, pixel_size=1.8 * _u.arcsec, beam_fwhm=5 * _u.arcsec, z=0.05,
):
    """
    ...
    old_x = (
        (sim_data.mx * sim_data.unit_length / kpc_per_arcsec).to(_u.arcsec).value
    )  # in arcsec
    old_z = (
        (sim_data.mz * sim_data.unit_length / kpc_per_arcsec).to(_u.arcsec).value
    )  # in arcsec
    ...
    sb_final = sb_smoothed.to(
        _u.mJy / _u.beam, equivalencies=_u.beam_angular_area(omega_beam)
    )
